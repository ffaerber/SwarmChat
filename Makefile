...
test-unit: ## Run only unit tests (no fork)
	forge test --match-contract ContactRegistryTest -vvv
...
abi: build ## Extract ABI to frontend
	python3 -c "import json; d=json.load(open('out/ContactRegistry.sol/ContactRegistry.json')); json.dump(d['abi'], open('frontend/src/abi/ContactRegistry.json','w'), indent=2)"
	@echo "ABI written to frontend/src/abi/ContactRegistry.json"
...
deploy-contract: build ## Deploy contract to Gnosis Chain
...
.PHONY: deploy-contract-chiado
deploy-contract-chiado: build ## Deploy contract to Chiado testnet
	@test -n "$(MNEMONIC)" || { echo "Error: set MNEMONIC in .env"; exit 1; }
	forge script script/Deploy.s.sol:DeployScript \
...
verify-contract: ## Verify existing contract on Blockscout (CONTRACT=0x...)
	forge verify-contract $(CONTRACT) src/ContactRegistry.sol:ContactRegistry \
		--rpc-url $(RPC_URL) \
		--verifier blockscout \
		--verifier-url https://gnosis.blockscout.com/api/
...
deploy-frontend: build-frontend ## Build + upload frontend to Swarm
	...
	@REFERENCE=$$(curl -s -X POST \
		"$(BEE_API_URL)/bzz?name=swarmchat" \
